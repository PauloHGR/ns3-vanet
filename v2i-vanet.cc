...
void ProcessedPacket (Ptr<Socket> socket)
{
	...
	arqWrite = fopen("/home/paulo/Documentos/mestrado/ads/Trabalho final/v2i-rx.csv","a");
	arq << "Car: " << socket->GetNode()->GetId() << " Receive of the car: " << seqTs.GetSeq ()
		<< "," << Now ().GetSeconds ()
		<< "," << Now ().GetSeconds ();
	fprintf(arqWrite,"%s\n",arq.str().c_str());
	fclose(arqWrite);
	//std::cout << "Car: " << socket->GetNode()->GetId() << " Received response packet: " << seqTs.GetSeq () <<" in: " << Now ().GetSeconds () << std::endl;
}
...
void ReceivePacket (Ptr<Socket> socket)
{
      ...
      FILE *arqWrite;
      std::ostringstream arq;
      arqWrite = fopen("/home/paulo/Documentos/mestrado/ads/Trabalho final/v2i.csv","a");
      arq << "Rsu: " << socket->GetNode()->GetId() << " Receive of the car: " << seqTs.GetSeq ()
				<< "," << Now ().GetSeconds ()
				<< "," << Now ().GetSeconds ();
      ...
      std::cout << "RSU: " << socket->GetNode()->GetId() << ", Receive of the car: " << seqTs.GetSeq () << ", Seconds: " << Now ().GetSeconds ()
      				<< ", Packet: " << pkt->GetUid()
					<< std::endl;
...
int main (int argc, char *argv[])
{
  ...
  MobilityHelper enb;
  Ptr<ListPositionAllocator> positionCar1 = CreateObject<ListPositionAllocator> ();
  positionCar1->Add (Vector (1550.0, 9000.0, 0.0)); //eNB0
  enb.SetMobilityModel ("ns3::ConstantPositionMobilityModel");
  enb.SetPositionAllocator(positionCar1);
  enb.Install (c.Get(50));
  ...
  Ptr<Socket> recvSink = Socket::CreateSocket (c.Get (50), tid);
  ...
  //AnimationInterface anim ("v2i.xml");
  Simulator::Run ();
  Simulator::Destroy ();
